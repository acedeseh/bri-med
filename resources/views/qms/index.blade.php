          ...
          <div class="col-sm-6 col-xl-3">
            <div class="card overflow-hidden rounded-2">
              <div class="card-body pt-3 p-4">
                <h6 class="fw-semibold fs-4">QMS</h6>
            ...
            <div id="qmsIssuePieChart">
              <canvas id="qmsPieChart"></canvas>
          </div>
          </div>     
                  ...
                  <div class="card-body">
                    <div class="d-sm-flex d-block align-items-center justify-content-between mb-9">
                      <div class="mb-3 mb-sm-0">
                        <h5 class="card-title fw-semibold">QMS Evaluation Overview</h5>
                      </div>
                      <div>
            ...
            <div class="card w-100">
              <div class="card-body p-4">
                ...
                <div class="container d-flex">              
                  <a class="btn btn-outline-secondary mx-3 mt-2 d-block mb-0 fs-3 ms-auto" href="{{ route('qms.create') }}" >Add</a>
                ...
                <div class="table-responsive">
                  <table class="table text-nowrap mb-0 align-middle">
                    ...
                    <tbody>
                      @foreach($qmsProblem as $qmsall)
                      <tr>
                        <td class="border-bottom-0 fw-semibold mb-1">{{ $qmsall->branchcode }}</td>
                        <td class="border-bottom-0 fw-semibold mb-1">{{ $qmsall->branchname }}</td>
                        <td class="border-bottom-0 fw-bold mb-1">{{ $qmsall->problem }}</td>
                        <td class="border-bottom-0 fw-semibold mb-1">{{ date('d/m/Y H:i', strtotime($qmsall->date_found)) }}</td>
                        <td class="border-bottom-0 fw-semibold mb-1">{{ date('d/m/Y H:i', strtotime($qmsall->sla_target)) }}</td>
                        <td class="border-bottom-0 fw-semibold mb-1">{{ date('d/m/Y H:i', strtotime($qmsall->date_done)) }}</td>
                        
                        <td class="border-bottom-0 fw-semibold">
                          @php
                              $slaTarget = strtotime($qmsall->sla_target);
                              $dateDone = $qmsall->date_done;
                        ...
                        <td class="border-bottom-0 fw-semibold mb-1">{{ $qmsall->issue }}</td>
                        <td class="border-bottom-0 fw-semibold mb-1">{{ $qmsall->analysis }}</td>
                        
                        <td class="border-bottom-0 fw-semibold mb-1 {{ $bgColor }}">
                          {{ $qmsall->status }}
                        </td>

                        <td class="border-bottom-0 fw-semibold mb-1">{{ $qmsall->note }}</td>
                        <th class="border-bottom-0">
                          <div class="btn-group">
                            <a  class="btn btn-warning" href="{{route('qms.edit', $qmsall->id)}}">Update</a>
                              <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#confirmDeleteModal{{ $qmsall->id }}">Delete</button>
                          </div>
                      </th>
                      </tr>

                        <div class="modal fade" id="confirmDeleteModal{{ $qmsall->id }}" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
                          <div class="modal-dialog modal-dialog-centered" role="document">
                              <div class="modal-content">
                                  ...
                                  <div class="modal-footer">
                                      ...
                                      <form method="POST" action="{{ route('qms.destroy', $qmsall->id) }}">
                                          @csrf
                                          @method('DELETE')
...
<script>
  ...
  window.location.href = "{{ url('/qms/edit') }}/" + id;
}
</script>
      ...
      $.ajax({
          ...
          success: function (data) {
              ...
              var qmsIssueConfig = {
                  ...
                  series: qmsIssueData.series,
                  labels: qmsIssueData.labels,
                  colors: ["#5D87FF", "#49BEFF", "#FF6384", "#36A2EB"], // Sesuaikan dengan warna yang Anda inginkan
              };

              // Buat pie chart di div dengan ID 'qmsPieChart'
              var qmsPieChart = new ApexCharts(document.querySelector('#qmsPieChart'), qmsIssueConfig);
              qmsPieChart.render();
          },
